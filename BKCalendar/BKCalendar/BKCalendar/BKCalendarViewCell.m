...
-(void)assignDataWithFirstDayInMonthDate:(NSDate *)date
{
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        NSInteger yearNumber = [[BKCalendar shareManager] getYearNumberWithDate:date];
        NSString * chineseYearStr = [[BKCalendar shareManager] getChineseYearNumberWithDate:date];
        ...
        NSInteger allMonthDayNum = [[BKCalendar shareManager] getNumberOfDaysInMonthDate:date];
        NSInteger weekNumber = [[BKCalendar shareManager] getNumberInWeekDate:date];
        
        CGFloat width = [UIScreen mainScreen].bounds.size.width/7.0f;
        CGFloat height = width;
        
        NSInteger monthNumber = [[BKCalendar shareManager] getMonthNumberInYearDate:date];
        
        dispatch_async(dispatch_get_main_queue(), ^{
           
            UILabel * monthTitleLab = [[UILabel alloc]initWithFrame:CGRectMake(weekNumber*width, 0, width, 30)];
            monthTitleLab.textColor = BKCalendarRedColor;
            monthTitleLab.font = [UIFont systemFontOfSize:16*[UIScreen mainScreen].bounds.size.width/414.0f];
        ...
        for (int i = 0; i<allMonthDayNum; i++) {
            ...
            NSDate * dayDate = [[BKCalendar shareManager] getDayDateInMonthDate:date andIndex:i+1];
            NSString * chineseNumber = [[BKCalendar shareManager] getChineseNumberInMonthDate:dayDate];
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                CGFloat X = i==0?weekNumber*width:CGRectGetMaxX(lastBtn.frame);
                CGFloat Y = i==0?30:CGRectGetMinY(lastBtn.frame);
                ...
                numberTitle.font = [UIFont systemFontOfSize:16*[UIScreen mainScreen].bounds.size.width/414.0f];
                ...
                chineseNumberTitle.font = [UIFont systemFontOfSize:10*[UIScreen mainScreen].bounds.size.width/414.0f];
                ...
                if (self.mark == 0) {
                    if ([nowDay intValue] == i+1) {
                        ...
                        colorView.backgroundColor = BKCalendarRedColor;
