...
-(void)showInView:(UIView *)supperView
{
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        contentRect.origin.y = self.frame.size.height - self.contentHeight;
...
-(void)hiddenComplete:(void (^)(void))complete
{
    ...
    [UIView animateWithDuration:0.3 animations:^{
        CGRect contentRect = self.contentView.frame;
        contentRect.origin.y = self.frame.size.height;
        self.contentView.frame = contentRect;
    } completion:^(BOOL finished) {
        self.userInteractionEnabled = YES;
        if (complete) {
...
-(void)resetDefaultPickerSelectFrame
{
    ...
    upLine.center = CGPointMake(upLine.center.x, self.defaultPicker.center.y - kRowHeight/2 - BK_PICKER_ONE_PIXEL);
